...
function getColor(size) {
    ...
    let color;
    try {
        if (colorStyle === 'style1') {
            color = color1;
        } else if (colorStyle === 'style2') {
            color = color2;
        } else if (colorStyle === 'style3') {
            color = color3;
        }
    } catch (err) {
...
function updateTable(person, bots) {
    ...
    if (cycleTable < TIME_SPACE_TABLE) cycleTable++;
        ...
        sortLimit.forEach((data, i) => {
            ...
            <td class="name player-name">${cubeName}</td>
...
settingsButton.addEventListener('click', () => {
    ...
    document.getElementById('webgl').style.zIndex = '5'; // Lower canvas z-index
});
...
saveSettingsButton.addEventListener('click', function() {
    ...
    document.getElementById('settingsForm').style.zIndex = -100;
    makeInitialFood();
});
