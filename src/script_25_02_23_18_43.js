...
const TIME_SPACE_BOT = 200;
const MAX_BOT = 7;
const MAX_FOOD = 6000;
const TIME_SPACE_FOOD = 20;
...
const maxWidth = sizes.width * 10;
const maxHeight = sizes.height * 10;
...
function getColor(size) {
  return color[Math.floor(Math.log2(size)) % 12 - 1];
}
...
class Cube {
  ...
  detectDirection() {
    ...
    if (this.enemies.length == 0) {
      if (this.food.length == 0) this.toRandom();//E: X, F: X
      else {//E: X, F: O
        if (prob5to5) this.toDistance(); // to distance minimum food
        else this.toSize(); //to size maximum food
      }
    } else {
