...
class Cube {
  ...
  eatFoodAround() {
    ...
    food.forEach((monster, i) => {
      ...
      if ((deltaX < minDist) && deltaY < minDist) {
        if (monster.size <= this.size) {
          ...
          if (monster.type == GHOST) bufGhost.push(i);
          else if (monster.type == BOT) bufBot.push(i - food.length);
          // else
        } else {
    ...
    if (buf.length != 0) {
      ...
      //   bot = deleteFromArray(bot, buf)
      //   if (i < food.length) food = deleteFromArray(food, buf[i]);
      //   else bot.deleteFromArray(i - fo)
      // }
      food = deleteIndexesFromArray(food, bufGhost);
      // bot = deleteIndexesFromArray(Bot, bufBot);
    }
  }
